...
from registro_classes import CLASSE_MAP
...
class App:
    def __init__(self):
        ...
        self.__botao_continuar_ref: Optional[pygame_menu.widgets.Button] = None
        self.__menu_principal = self._criar_menu_principal()

    # --- GETTERS E SETTERS PARA A CLASSE APP ---
    ...
    def _criar_menu_ranking(self) -> pygame_menu.Menu:
        ...
        def atualizar_ranking(menu_atual, proximo_menu):
            ranking_manager = self.get_ranking_manager() 
    ...
    def _iniciar_jogo_callback(self, carregar_save=False):
        self.get_menu_principal().disable() 
        gerenciador_som = self.get_gerenciador_som()
        ...
        self.get_menu_principal().enable() 
    ...
    def _coletar_nome_jogador_e_salvar_score(self, pontuacao_final: int):
        ...
        menu_nome.mainloop(self.get_tela()) 
    ...
    def _atualizar_botao_continuar(self):
        botao = self.get_botao_continuar_ref() 
        if not botao: return
        ...
        botao.readonly = not eh_valido

    def run(self):
        self.get_gerenciador_som().tocar_musica_fundo('menu')
        self._atualizar_botao_continuar()
        try:
            self.get_menu_principal().mainloop(self.get_tela())
